...
- 2026-01-25 v1.37：优化 Codex 平台差异条目，去除重复表述并收敛注意事项。
...
- 2026-01-25 v1.27：强化中文沟通口径；扩展 Codex 项目内平台差异条目。
- 2026-01-25 v1.26：补充 Codex 平台排查规则加载的实践条目。
- 2026-01-25 v1.25：补充 Codex 项目内平台实践差异条目。
...
## B. 平台差异（Codex 项目内）
### B.1 加载与作用域
- 最小摘要：加载=全局 `~/.codex`(override>AGENTS.md)；项目=override>AGENTS.md>fallback。
- 操作：加载范围有疑问先对照 GlobalUser/AGENTS.md 的 B.1。
- 风险提示：文件名不在 fallback 列表时可能不被加载，需核对加载链条。
- 回滚提示：临时覆盖结束后删除或回退 `AGENTS.override.md`，并确认项目层加载顺序恢复。
### B.2 配置与上限
- 最小摘要：上限=32768；配置=`~/.codex/config.toml`（字段以文档为准）。
- 操作：调整配置或上限前对照 GlobalUser/AGENTS.md 的 B.2。
- 风险提示：配置字段或上限变更会影响加载结果，需先对照文档确认。
- 回滚提示：配置试验完成后恢复既有基线值，并记录回滚前后差异。
### B.3 诊断与命令
- 最小摘要：诊断=`codex status` / `codex --ask-for-approval never "Summarize the current instructions."` / `codex --cd subdir --ask-for-approval never "Show which instruction files are active."`。
- 操作：排查问题先跑上述命令并记录结论，再对照 GlobalUser/AGENTS.md 的 B.3。
- 风险提示：诊断命令仅用于确认当前加载状态，不会自动修复规则。
- 回滚提示：若排查过程引入临时配置或覆盖文件，结论确认后立即回退并复测。
### B.4 注意事项
- 注意事项：本项目小改优先 `apply_patch`，跨多文件改动优先脚本以便复查。
- 注意事项：如需临时覆盖规则，使用 `AGENTS.override.md` 并说明影响与回滚。
- 注意事项：本节仅写项目内 Codex 细节，避免重复全局规则。
- 回滚提示：临时流程或说明应在任务结束时恢复默认，避免沉淀为常驻规则。
