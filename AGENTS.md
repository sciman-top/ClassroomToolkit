...
## B. 平台差异（Codex 项目内）
### B.1 加载与作用域
- 最小摘要：优先级 `AGENTS.override.md` > `AGENTS.md` > fallback。
- 操作：规则生效异常时先对照 `GlobalUser/AGENTS.md` B.1。
- 回滚：临时 override 完成后删除并复测加载链。

### B.2 配置与上限
- 最小摘要：配置文件常见位置 `~/.codex/config.toml`。
- 操作：调整上限与 fallback 前先确认官方文档字段。
- 回滚：保留变更记录并恢复基线值。

### B.3 诊断与命令
- 最小摘要：优先用 `codex status` 与非交互命令做规则诊断。
- 操作：先诊断再修正，避免盲改规则文件。
- 回滚：清理排查时引入的临时配置。
