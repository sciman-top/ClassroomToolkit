...
- 操作：规则未生效时，先检查工作目录、`.geminiignore`、`.gitignore` 与全局加载说明。
- 回滚：任务结束后回退临时 ignore、临时上下文文件名或临时扩大扫描范围的设置。

### B.2 配置与上限
- 最小摘要：context discovery 相关上限与字段以官方文档为准。
- 操作：修改配置前后执行 memory 刷新，并记录实际生效范围、验证方式与回滚点。
- 回滚：试验配置结束后恢复基线值，并复测当前仓库上下文是否正常。

### B.3 诊断与命令
- 最小摘要：优先通过 memory 或等效命令确认当前上下文。
- 操作：若发现上下文偏差，先刷新并排查目录/ignore，再决定是否修正规则文件。
- 回滚：清理排查期加入的临时记忆、临时上下文说明与试验配置。

### B.4 注意事项
- 小改优先 `apply_patch`，跨多文件批改优先脚本化，便于复核。
- 本节仅保留 Gemini 在本仓库的差异，不复制全局共性条款。
