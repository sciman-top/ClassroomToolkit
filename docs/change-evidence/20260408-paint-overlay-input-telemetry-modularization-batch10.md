...
当前落点=PaintOverlayWindow 输入处理（遥测与抑制辅助）
目标归宿=将输入遥测与手势抑制辅助方法拆分到独立 partial，进一步收敛 Input 主文件职责
迁移批次=2026-04-08-maintainability-hardening-batch10
风险等级=低
执行命令=
...
- 热点体量下降：PaintOverlayWindow.Input.cs 从 1650 预算基线下降到 1112（delta -538）
回滚动作=
