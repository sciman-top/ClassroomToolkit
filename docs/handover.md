...
- 本轮 full Debug 已通过 2997 个普通/性能测试、29 个核心契约、漏洞与 analyzer；这仍是 `repo_verified`，不是 `live_accepted`。
...
- 后续只有出现独立失败或触及相应模块时，再按 deletion test 局部内联浅 policy，或把静态安全契约替换为行为测试；有生产 adapter + 测试 adapter 的 Interop seam 保留。
