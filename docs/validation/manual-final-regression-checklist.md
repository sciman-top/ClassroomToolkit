...
- 场景矩阵（每组至少 20 次连续翻页）：
  ...
  - `WPS 全屏` + `光标模式` + `滚轮翻页`
- 切换回归链路：
  - 在放映中执行 `画笔 -> 光标 -> 画笔 -> 光标` 循环切换（至少 10 轮）。
  - 每轮切换后立即执行 `键盘翻页 + 滚轮翻页`，确认无失效与无模式卡死。
- 延迟与手感门禁：
  - 光标模式键盘翻页不得出现稳定可感知的额外队列延迟。
  - 若出现延迟，记录发生次数、模式、输入源、应用（PPT/WPS）与日志标签。
- 证据模板（每条用例至少记录一次）：
  - `应用=`；`模式=`；`输入源=`；`切换前状态=`；`切换后状态=`；`连续翻页次数=`；`是否失败=`；`日志片段=`；`截图/录屏路径=`
