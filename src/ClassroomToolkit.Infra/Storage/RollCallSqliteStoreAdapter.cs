...
public sealed class RollCallSqliteStoreAdapter : IRollCallWorkbookStore
{
    ...
    private string ResolveDbPathSafe(string workbookPath)
    {
        try
        {
            ...
            Debug.WriteLine("[RollCallSqlite] resolver returned empty path; fallback to default path policy.");
        }
        catch (Exception ex) when (InfraExceptionFilterPolicy.IsNonFatal(ex))
        {
            Debug.WriteLine($"[RollCallSqlite] resolver failed: {ex.GetType().Name} - {ex.Message}");
        }
    ...
    private static string ResolveDbPath(string workbookPath)
    {
        ...
        {
            return Path.Combine(AppContext.BaseDirectory, $"{fallbackFileName}.rollcall.sqlite3");
        }
    }
