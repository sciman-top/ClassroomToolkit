...
public sealed class RollCallSqliteStoreAdapterTests
{
    [Fact]
    ...
    [Fact]
    public void LoadOrCreate_ShouldBootstrapSqlite_WhenBridgeContainsState()
    {
        var workbook = CreateWorkbook();
        var bridge = new FakeRollCallStore(new RollCallWorkbookStoreLoadData(workbook, CreatedTemplate: false, RollStateJson: "{\"from\":\"excel\"}"));
        var dbPath = CreateTempDbPath();
    ...
    [Fact]
    public void Save_ShouldClearSqliteState_WhenRollStateIsNull()
    {
        var workbook = CreateWorkbook();
        var bridge = new FakeRollCallStore(new RollCallWorkbookStoreLoadData(workbook, CreatedTemplate: false, RollStateJson: null));
        var dbPath = CreateTempDbPath();
        SeedSqliteState(dbPath, "{\"state\":1}");
        var adapter = new RollCallSqliteStoreAdapter(bridge, _ => dbPath);

        adapter.Save(workbook, "students.xlsx", null);

        ReadSqliteState(dbPath).Should().BeNull();
    }
    ...
    public void LoadOrCreate_ShouldFallbackToSqliteSnapshot_WhenBridgeThrows()
    {
        ...
        var dbPath = CreateTempDbPath();
        var seedingBridge = new FakeRollCallStore(new RollCallWorkbookStoreLoadData(workbook, CreatedTemplate: false, RollStateJson: "{\"seed\":1}"));
        var seedingAdapter = new RollCallSqliteStoreAdapter(seedingBridge, _ => dbPath);
        seedingAdapter.Save(workbook, "students.xlsx", "{\"seed\":1}");

        var failingBridge = new ThrowingRollCallStore();
        var adapter = new RollCallSqliteStoreAdapter(failingBridge, _ => dbPath);
    ...
    {
        var dbPath = CreateTempDbPath();
        SeedRollCallSnapshotWorkbook(dbPath,
            """
            ...
            """);
        var adapter = new RollCallSqliteStoreAdapter(new ThrowingRollCallStore(), _ => dbPath);
    ...
    {
        var dir = TestPathHelper.CreateDirectory("ctool_rollcall_sqlite");
    ...
    {
        private readonly RollCallWorkbookStoreLoadData _loadResult;

        public FakeRollCallStore(RollCallWorkbookStoreLoadData loadResult)
        {
    ...
    private sealed class ThrowingRollCallStore : IRollCallWorkbookStore
    {
        public RollCallWorkbookStoreLoadData LoadOrCreate(string path)
        {
